...
export class Tezos {
  transport: Transport<*>;
    ...
    constructor(transport: Transport<*>, buffer) {
  ...
  async sign(
    ...
    for (let i = 0; i < toSend.length; i++) {
      ...
      response = await transport.send(0x80, apdu, code, curve, data);
    }
    let signature;
    if (curve == 0) {
  ...
  async signOperation(
    ...
    curve = curve ? curve : 0x00;
      const result = await this.sign(path, rawTxHex, curve, 0x04, this.transport);
    return result;
  }
  ...
  async signHash(
    ...
    curve = curve ? curve : 0x00;
    const result = await this.sign(path, rawTxHex, curve, 0x05, this.transport);
    return result;
  }
